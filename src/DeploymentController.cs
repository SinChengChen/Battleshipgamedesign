	...
	public static void DrawDeployment()
	{
		...
		if (GameResources.ThemeState) {
			...
			if (_currentDirection == Direction.LeftRight) {
				SwinGame.DrawBitmap (GameResources.GameImage ("LeftRightButton2"), LEFT_RIGHT_BUTTON_LEFT, TOP_BUTTONS_TOP);
				...
				SwinGame.DrawBitmap (GameResources.GameImage ("UpDownButton2"), LEFT_RIGHT_BUTTON_LEFT, TOP_BUTTONS_TOP);
		...
		foreach (ShipName sn in Enum.GetValues(typeof(ShipName))) {
			...
			if (i >= 0) {
				if (sn == _selectedShip) {
					if (GameResources.ThemeState) {
						SwinGame.DrawBitmap (GameResources.GameImage ("SelectedShip2"), SHIPS_LEFT, SHIPS_TOP + i * SHIPS_HEIGHT);
		...
		if (GameController.HumanPlayer.ReadyToDeploy) {
			if (GameResources.ThemeState) {
				SwinGame.DrawBitmap (GameResources.GameImage ("PlayButton2"), PLAY_BUTTON_LEFT, TOP_BUTTONS_TOP);
				...
				SwinGame.DrawBitmap(GameResources.GameImage("RandomButton2"), RANDOM_BUTTON_LEFT, TOP_BUTTONS_TOP);
			} else { 
